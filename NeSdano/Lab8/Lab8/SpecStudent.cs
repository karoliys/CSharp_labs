...
namespace Lab3
{
    struct MarksOfBsuir
    {
        ...
        public void RandMark(ref int[] predmet)
        {
            ...
            if (predmet != null)
            {
                ...
                predmet[predmet.Length - 1] = random.Next(3, 8);
            }
            else
            {
                predmet = new int[1];
                predmet[0] = random.Next(3, 8);
            }
    ...
    class SpecStudent : Student
    {
        ...
        private string Spec;
        public event AccountHandler ChanceEvent;
        public override void Status()
        {
            StatusDelegate StatusTwo= () => Console.WriteLine("student is in faculty:" + Faculty + " and specialization: " + Spec);
            StatusTwo();
            Console.ReadKey();
        }
        public SpecStudent() { }
        ...
        public void SdatZachet()
        {
            Console.Clear();
            ChanceEvent += DisplayRedMessage;
            double chance = 0;
            int kolvo = 0;
            if (marks.MMA != null)
            {
                for (int i = 0; i < marks.MMA.Length; i++)
                {
                    chance += marks.MMA[i];
                    kolvo++;
                }
            ...
            if (marks.ISP != null)
            {
                for (int i = 0; i < marks.ISP.Length; i++)
                {
                    chance += marks.ISP[i];
                    kolvo++;
                }
            ...
            if (marks.OAIP != null)
            {
                for (int i = 0; i < marks.OAIP.Length; i++)
                {
                    chance += marks.OAIP[i];
                    kolvo++;
                }
            ...
            if (kolvo != 0)
            {
                chance = chance / kolvo * 10;
                ChanceEvent?.Invoke("Chance = " + chance + "%");
            }
            else
            {
                ChanceEvent?.Invoke("Chance 0");
            }
            ChanceEvent -= DisplayRedMessage;
            Console.ReadKey();          
        }
        public override void MethodsInvocation()
        {
            ...
            while (retry)
            {
                ...
                switch (answer)
                {
                    ...
                    case '8': SetScoreCT(); SetAttestat(); break;
                    case '9': SdatZachet(); break;
                    default:
                        retry = false;
                        break;
                }
